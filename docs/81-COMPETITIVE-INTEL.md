...
> **Status: P1 + P2 built.** A market is modelled as tracked brands, weighted
> dimensions, an evidence register with full provenance, and 1–5 scores that
> are **refused unless evidence backs them**. All three SOW deliverables
> generate from stored evidence: the executive scorecard (markdown or a
> four-sheet XLSX workbook), month-over-month material-change detection, and
> the board report. Autonomous collection is P3.
>
...
watch_evidence  action=add …                        # collect (agent + operator)
watch_score     subject=… dimension=… score=4       # refused without evidence
watch_scorecard format=xlsx path=~/YSI-scorecard.xlsx
watch_board_report path=~/YSI-board-march.md        # snapshots on the way out
```

## Roadmap

- **P3** — `watch_observe` autonomous collection passes, a refresh queue driven
  by the staleness data above, per-state proxy pool and personas, scheduled
  weekly / monthly / quarterly runs.
