...
def _mock_httpx(mock_response):
    """Set up a patched httpx.AsyncClient context manager."""
...
class TestOllamaEmbedder:
    ...
    async def test_embed_returns_vector(self) -> None:
        ...
        with patch("core.embeddings.httpx.AsyncClient") as mock_cls:
            mock_cls.return_value = _mock_httpx(_ollama_response(fake_vec))
    ...
    async def test_embed_error_raises(self) -> None:
        ...
        with patch("core.embeddings.httpx.AsyncClient") as mock_cls:
            mock_cls.return_value = _mock_httpx(mock_resp)
...
class TestOpenRouterEmbedder:
    ...
    async def test_embed_returns_vector(self) -> None:
        ...
        with patch("core.embeddings.httpx.AsyncClient") as mock_cls:
            mock_cls.return_value = _mock_httpx(_openrouter_response([fake_vec]))
            result = await client.embed("test text")
    ...
    async def test_embed_batch(self) -> None:
        ...
        with patch("core.embeddings.httpx.AsyncClient") as mock_cls:
            mock_cls.return_value = _mock_httpx(_openrouter_response(fake_vecs))
            results = await client.embed_batch(["text1", "text2"])
    ...
    async def test_sends_auth_headers(self) -> None:
        ...
        with patch("core.embeddings.httpx.AsyncClient") as mock_cls:
            mock_instance = _mock_httpx(_openrouter_response([fake_vec]))
            mock_cls.return_value = mock_instance
            await client.embed("test")
...
class TestCreateEmbedder:
    ...
    def test_openrouter_requires_api_key(self, test_config) -> None:
        """Factory raises if OpenRouter selected but no API key."""
        test_config.knowledge.embedding_provider = "openrouter"
        test_config.llm.providers.pop("openrouter", None)
        with pytest.raises(RuntimeError, match="requires an API key"):
            create_embedder(test_config)
