...
class TestFieldChanges:
    def test_newest_in_window_vs_newest_before_and_new_coverage(self) -> None:
        since = _iso(7)
        ev = [  # newest first
            {"subject": "Brand C", "dimension": "Promotions", "subcriterion": "welcome", "claim": "200% extra", "value_text": "200%", "observed_at": _iso(1), "source_url": "u1"},
            {"subject": "Brand C", "dimension": "Promotions", "subcriterion": "welcome", "claim": "150% extra", "value_text": "150%", "observed_at": _iso(3), "source_url": "u1b"},
            {"subject": "Brand C", "dimension": "Promotions", "subcriterion": "welcome", "claim": "150% extra", "value_text": "150%", "observed_at": _iso(20), "source_url": "u0"},
            {"subject": "Brand C", "dimension": "Loyalty", "subcriterion": "tiers", "claim": "5 tiers", "value_text": "5", "observed_at": _iso(2), "source_url": "u2"},
            {"subject": "Brand C", "dimension": "Loyalty", "subcriterion": "tiers", "claim": "5 tiers", "value_text": "5", "observed_at": _iso(30), "source_url": "u2"},
            {"subject": "Brand N", "dimension": "Packages", "subcriterion": "min", "claim": "$4.99", "value_text": "4.99", "observed_at": _iso(1), "source_url": "u3"},
        ]
        got = field_changes(ev, since=since)
        by = {(c["brand"], c["dimension"]): c for c in got}
        assert by[("Brand C", "Promotions")]["before"] == "150%" and by[("Brand C", "Promotions")]["after"] == "200%"
        assert not by[("Brand C", "Promotions")]["new_coverage"]
        assert ("Brand C", "Loyalty") not in by  # unchanged
        assert by[("Brand N", "Packages")]["new_coverage"] and by[("Brand N", "Packages")]["before"] is None

    def test_offer_changes_before_after(self) -> None:
        since = _iso(7)
        card = {"rows": [{"name": "Brand C", "is_self": False, "rank": 1, "overall": {"normalized_pct": 60.0}}]}
        ev = [
            {"subject": "Brand C", "dimension": "Promotional proposition", "claim": "New players get 200% extra Gold Coins on first purchase.", "value_text": "", "observed_at": _iso(1), "source_url": "u1"},
            {"subject": "Brand C", "dimension": "Promotional proposition", "claim": "New players get 150% extra Gold Coins on first purchase.", "value_text": "", "observed_at": _iso(20), "source_url": "u0"},
        ]
...
class TestBrief:
    ...
    async def test_build_render_and_tool(self, wm, tmp_path) -> None:
        ...
        ll = await wm.add_subject(company_id="c1", name="Brand H")
        ...
        await wm.add_evidence(company_id="c1", subject_id=ll.subject_id, dimension_id=states, subcriterion="states",
                              claim="Brand H is closing on September 14, 2026.", value_text="2026-09-14",
                              observed_at=_iso(1), source_url="https://ll")
        brief = await build_weekly_brief(wm, "c1", request="Is Brand C's welcome offer bigger than ours?",
        ...
        assert len(real) == 1 and real[0]["brand"] == "Brand C" and real[0]["after"] == "200%"
        assert brief["offer_changes"] and brief["offer_changes"][0]["brand"] == "Us"  # ours listed first
        assert brief["market_events"][0]["brand"] == "Brand H"
        facts = brief_facts(brief)
        assert "1 field change" in facts["changed"][0]
        assert any("Brand H" in m for m in facts["market"])
        assert any("Decide the response to Brand H" in d for d in facts["decisions"])
        md = render_brief_markdown(brief, {})
        assert "# Weekly competitive brief" in md and "| Brand C | Promotional proposition and generosity · welcome | 150% | 200% |" in md
        assert "Is Brand C's welcome offer bigger" in md
        p = render_brief_slide(brief, {"headline": "Brand C raised its welcome offer to 200%"}, path=tmp_path / "b.pptx")
        prs = Presentation(p)
        assert len(prs.slides) == 1
        txt = "\n".join(sh.text_frame.text for sh in prs.slides[0].shapes if sh.has_text_frame)
        assert "Brand C raised its welcome offer to 200%" in txt and "DECISIONS" in txt.upper()
...
class TestAlerts:
    def test_candidates_from_events_voice_and_regulatory(self) -> None:
        ev = alerts_from_events([{"brand": "Brand H", "claim": "Brand H is closing on September 14, 2026.", "url": "u"}])
        assert ev[0]["kind"] == "market_event" and ev[0]["dedupe_key"]
        voice = {"since": _iso(7), "window_days": 7, "brands": [
            {"name": "Brand C", "too_few": False, "themes": {"redemption_speed": {"n": 12, "share": 0.5, "neg_share": 0.8}}},
            {"name": "Spin", "too_few": False, "themes": {"support": {"n": 12, "share": 0.5, "neg_share": 0.8}}},
        ]}
        vdiff = {"baseline": False, "changed": [
            {"brand": "Brand C", "theme": "redemption_speed", "neg_from": 0.4, "neg_to": 0.8, "direction": "rising"}]}
        vs = alerts_from_voice(voice, vdiff)
        assert [a["subject_name"] for a in vs] == ["Brand C"]  # Spin: no rise recorded
        first = alerts_from_voice(voice, {"baseline": True, "changed": []})
        assert {a["subject_name"] for a in first} == {"Brand C", "Spin"}
    ...
    async def test_check_stores_once_and_reports_notify_state(self, wm) -> None:
        ...
        await wm.add_evidence(company_id="c1", subject_id=ll.subject_id, dimension_id=dim.dimension_id,
                              subcriterion="states", claim="Brand H is closing on September 14, 2026.",
        ...
        assert gw.sent and gw.sent[0].data["notification_type"] == "watch" and "Brand H" in gw.sent[0].data["text"]
