...
def _card():
    return {
        "rows": [
            ...
            {"name": "Brand C", "is_self": False, "rank": 2, "provisional": False,
...
def _voice(*, crown_n=20):
    ...
    return {
        ...
        "brands": [
            ...
            {"subject_id": "cc", "name": "Brand C", "is_self": False, "n": crown_n, "too_few": False,
...
class TestVoiceOnTop:
    def test_voice_adds_the_slides_and_the_profile_strip(self, tmp_path) -> None:
        ...
        narrative["profiles"] = [{"brand": "Brand C", "title": "Brand C", "observations": ["o1", "o2"], "implications": ["i1"]}]
        out = tmp_path / "v.pptx"
        ...
        assert "Brand C" in heat and "too few (n<" in heat          # short, so the row stays one line
    ...
    def test_movement_slide_lists_rising_and_falling(self, tmp_path) -> None:
        ...
        vdiff = {"baseline": False, "material_count": 1, "changed": [
            {"brand": "Brand C", "is_self": False, "theme": "redemption_speed", "n_from": 5, "n_to": 10,
        ...
        assert "▲ Brand C – Redemption speed: 20% → 50% of mentions, 50% → 90% negative (n 5 → 10)" in mv
...
class TestStandalone:
    def test_voice_deck_and_workbook_on_their_own(self, tmp_path) -> None:
        ...
        assert any("Brand C – players complain about redemption speed" in t for t in texts)
        ...
        rows = [{"brand": "Brand C", "source": "reddit", "posted_at": "2026-08-10", "theme": "redemption_speed",
    ...
    def test_scorecard_workbook_gains_a_voice_sheet_only_when_rows_given(self, tmp_path) -> None:
        ...
        b = render_scorecard_xlsx(card, dimensions=[], evidence=[], staleness=[], path=tmp_path / "b.xlsx",
                                  voice_rows=[{"brand": "Brand C", "source": "reddit", "quote": "q", "theme": "support",
                                               "sentiment": "negative"}])
...
class TestReportTools:
    ...
    async def test_board_report_has_no_voice_section_until_rows_exist_then_auto_adds_it(self, wm, tmp_path) -> None:
        ...
        md = r1.data["markdown"]
        assert "## What players say" in md and "| Brand C | 16 | 100% | redemption speed |" in md
