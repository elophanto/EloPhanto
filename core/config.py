...
@dataclass
class MindArbiterConfig:
    """Scored-candidate arbiter for the autonomous mind (Phase 3 —
    docs/75-AUTONOMOUS-MIND-V2.md). When enabled (default) the mind
    picks an action from a ranked candidate menu generated by typed
    sources (workable_checkpoints, mission_momentum, dream, reflexes,
    external_signals). When disabled, falls back to the legacy
    free-form dream / goal prompt — kept as an escape hatch.

    Default flipped to True on 2026-05-20 after the AlphaScala
    log review showed the legacy prompt produces analysis-paralysis
    loops ("12 steps of thinking about collecting 25 sources"). The
    arbiter replaces that with a forced pick from a scored menu and
    grounds every wakeup in actual state.
    """
    ...
    weights: dict[str, float] = field(
        default_factory=lambda: {
            ...
            # meaning of each. Defaults are tuned so a moderate
            # checkpoint scores ~4-5 and a high-leverage stale
            # mission move scores ~7-9.
            "value": 1.0,
            "lens_bonus": 0.6,
            "staleness_bonus": 0.4,
            "affect_bias": 1.0,
...
def load_config(config_path: Path | str | None = None, profile: str = "") -> Config:
    ...
    # Arbiter sub-block (Phase 3). Defaults stay opt-in: missing
    # config keeps the legacy free-form dream/goal prompt.
    arb_raw = am_raw.get("arbiter", {}) if isinstance(am_raw, dict) else {}
