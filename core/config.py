...
class MindArbiterConfig:
    ...
    docs/75-AUTONOMOUS-MIND-V2.md). When disabled the mind uses the
    legacy free-form dream / goal prompt; when enabled it picks an
    action from a ranked candidate menu generated by typed sources
    (workable_checkpoints, mission_momentum, dream, reflexes,
    external_signals).

    Off by default so the rollout is opt-in. Flip to True after the
    arbiter has been observed in dry-run logs for a wakeup cycle or
    two — the dashboard / mind status surface will eventually show
    the candidate menu the arbiter saw on the last cycle.
    """
