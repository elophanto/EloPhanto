...
async def build_role_roster_context(
    *,
    ...
    return (
        ...
        f"{roster}\n"
        "\n"
        "When a reply covers work that spans these functions, ATTRIBUTE each part "
        "to the hat that did it: prefix that part with the role's emoji and its "
        'title exactly as listed above — e.g. "📣 Marketing — drafted 3 replies." '
        "This lets the operator "
        "watch the team work, which is how an autonomous business earns trust.\n"
        "Rules: label only REAL role-work; never invent a role, pad, or split a "
        "trivial reply to look busy; a single-topic answer needs no labels. "
