...
**在动用资金、接触密钥或改写自身之前，它会停下来征求批准。** 正是这一条规则，让"把它一直开着"成为一个合理的决定。
...
`knowledge/self/nature.md` 不由任何人撰写，而是智能体基于自身可度量的结果反思后自行维护的文件。全新安装时它并不存在，会随着工作逐步被写出来。以下是某个运行中实例的当前文件，原文引用：
...
这里的自主性是逐级获得的。一家公司要从 `learning` 一路挣到 `trial`、再到 `operating`；一旦进入 operating，对外触达与销售便由它自己运作。而动用资金始终是 CRITICAL 级动作，除 `nuclear` 外的任何模式下都会逐次询问。加密货币是 Solana 与 Base 上的真实自托管通道；Stripe 法币通道则以测试模式交付，直到你完成 KYC 并手动切换。日历信号来自 ICS 文件与 webhook，而不是一个 Google OAuth 按钮。自我修改是它在获批后进入的流程，绝不会是无声的改写。另外，一个始终在线的智能体会消耗真实的 token，先盯一周账本，再决定要不要放宽预算。
