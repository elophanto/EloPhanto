...
[![The Agent Society — an isometric campus with residents at work across six departments](docs/images/agent-society.png)](docs/93-AGENT-SOCIETY.md)
...
8. **An honest answer about what it's doing.** The goal runner resumes an active goal on every startup, and the heartbeat and mind run on their own timers. Ask what's running and it reports all of them with live state — not just the spawned agents — and hands you the command to stop each one. `stop` cancels the current run; `stop --hard` halts everything.
