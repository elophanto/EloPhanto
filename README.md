...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-2080%2B-success" alt="Tests">
...
- **Identity** — values, beliefs, and capabilities discovered through reflection. The operator names the agent in the setup wizard (default `EloPhanto`, any string works — `Riley`, `AlphaScala`, your fan's name). The agent writes a `nature.md` it edits over time, surfaces the configured name in every system prompt, and rename-sweeps the self-narrative files on startup when `config.agent_name` changes — so a rename actually propagates end-to-end (DB, knowledge chunks, dashboard, LLM context) without a reinstall.
...
- **Affect** — state-level emotion based on [Mehrabian's PAD model (1980)](https://en.wikipedia.org/wiki/PAD_emotional_state_model) substrate plus [OCC (1988)](https://en.wikipedia.org/wiki/Ortony,_Clore,_and_Collins_model) appraisal labels. Three continuous channels (Pleasure, Arousal, Dominance) decay toward zero on the order of minutes-to-hours. Events fire from operator surfaces — corrections → frustration (or anger on high-severity repeats), tool failures → anxiety, verification PASS → relief, checkpoint hits → pride, long idle → restlessness — **and from content the agent reads autonomously** through two paths. The LLM-callable `affect_record_event` tool catches sarcasm and coded language the LLM notices. Underneath sits a **mechanical executor-side content inference** layer (Phase 6) — a high-precision regex catalog runs on every successful content-yielding tool result (`browser_extract`, `email_read`, etc.) and emits affect events directly without asking the LLM. Patterns carry severity weights (seed-phrase ask = 1.5×, "delete this" = 0.5×) plus a self-relevance amplifier (agent name near match = +1.5×). Phishing directed at the agent fires at 2.25× — the loudest single signal the substrate can receive. Sources tag as `content:browser` or `content:email` so operators can answer "where did this mood come from?". Without the content path the affect system was deaf in autonomous mode (`affect_record_event` was called zero times in 17h of production; the mechanical channel underneath now closes that gap). Repeat events compound (3 corrections in 5 min hit harder than 3 spread across a day). Guidance is **directive, not permissive** — "you ARE feeling frustration at moderate intensity" with per-label TONE + BEHAVIOR cues (e.g. frustration → *shorter sentences, less hedging; do NOT paper over the friction by switching tasks*), so the LLM's helpful-by-default training can't drown out the felt state. Affect biases router temperature ±0.2, colors the system-prompt tone block, and feeds back into ego recompute so trait-level self-image reflects state-level feeling. **Sister to ego, different timescale**: ego is who I've become; affect is who I am right now. Inspect with `elophanto affect status`; smoke-test trajectories with `elophanto affect simulate <scenario>` (12 scenarios including content-source `scam-dm-stream`, `hostile-replies`, `warm-stream`, `autonomous-day`, `correction-during-scam`). See [docs/69-AFFECT.md](docs/69-AFFECT.md) and [core/affect.py](core/affect.py).
- **Autonomous Mind** — a data-driven background loop that runs between your messages. Queries real system state (goals, schedules, memory, knowledge, identity, ego, affect) to decide what to do next. Self-bootstraps on first boot, every tool call visible in real time. When no goals exist, enters **dream phase**: rotates daily across seven value lenses (compounding / capability / research / relational / creation / identity / infrastructure), so the agent doesn't get stuck in one mode of thinking. The dream tool injects installed skills (parsed from each `SKILL.md` frontmatter), an environmental snapshot (affect / ego / planning state), and a `PREVIOUSLY PROPOSED` block from the persistent **dream journal** so the next cycle can't re-propose ideas it already considered. Candidates run through pre-persist **dedup** (cosine similarity against existing goals via embeddings) — near-duplicates of recent goals get dropped before they hit the journal. The **goal-completion → instant wakeup hook** collapses the next cycle to seconds when a goal finishes, so dream fires immediately instead of waiting for the next scheduled tick. A mechanical **FORCE-DREAM gate** in `_build_prompt` prepends a hard directive when zero workable goals exist — the LLM can no longer wander off into email or file reads. **Planning-stuck retry** in the maintenance loop auto-retries decompose after 30 min stuck, cancels after 2 retries, so a poisoned `[GOAL-PLANNING]` state doesn't block dream forever.
...
> ABE — Autonomous Business Entity — is a concept originated by
> Petr Royce in 2023. The implementation in this codebase was built
...
That's the entire happy path. **Don't copy `config.demo.yaml` manually** — `setup.sh` runs `elophanto init` for you, which: (1) asks for the agent's **name** (default `EloPhanto`; any string works — propagates to display name, workspace path, email from-name, hub registration); (2) auto-installs Node.js + ffmpeg via `brew` when missing on macOS; (3) auto-detects your Chrome profile; (4) asks for at most one API key (`OpenRouter` is the easiest, or it auto-uses your ChatGPT Plus/Pro subscription via Codex if `~/.codex/auth.json` is present — no per-call API spend in that case); (5) generates the Ed25519 agent identity proactively; (6) prompts for vault initialisation. Manually copying the demo file and forgetting to replace `YOUR_OPENROUTER_KEY` is the #1 reason new installs fail silently.
...
> **Want it to actually work while you sleep?** Run with `./start.sh --daemon`. That installs EloPhanto as a launchd / systemd service so the **autonomous mind keeps thinking after you close the terminal** — it wakes on its configured interval (default 4–5 min), notices when goals complete, dreams up new ones across the seven value lenses, and runs the work. Without `--daemon`, the mind only runs while the terminal is open. The agent only "lives on its own" if you let it.
...
**Day 1 — blank slate.** You name the agent in the setup wizard (default `EloPhanto`). The identity layer writes its first `nature.md` and stores the chosen name in the DB; `<agent_identity>` in every system prompt addresses the agent by that name. Tool registry shows 200+ entries it hasn't used. Ego layer empty — coherence 1.0, no measured confidence, no humbling events. `permission_mode: ask_always`; you approve every risky action.
...
- **Skills + EloPhantoHub** — 172+ bundled best-practice skills across 9 divisions (engineering, design, marketing, product, project management, support, testing, specialized, spatial computing), 27 Solana ecosystem skills (DeFi, NFTs, oracles, bridges, security — sourced from [awesome-solana-ai](https://github.com/solana-foundation/awesome-solana-ai)), the NEXUS strategy system (7-phase playbooks, 4 scenario runbooks), 75 organization role templates for specialist spawning, and a public skill registry for searching, installing, and sharing skills. Plus **skill promotion** — the `skill_promote` tool distils 2–30 related lesson markdowns (from `knowledge/learned/lessons/`) into one reusable `SKILL.md`, closing the loop between micro-pattern accrual and named, retrievable capability. Lessons crystallize into skills the dream tool surfaces next cycle. Workspace artifacts auto-index too: every `file_write` to a `.md` file in the agent's configured workspace flows through the knowledge indexer, so deliverables become `knowledge_search`-retrievable next session instead of scattering into 1000+ orphaned files.
...
- **Web dashboard** — full monitoring UI at `localhost:3000`, at parity with the CLI surface. A **living agent "core"** (a luminous orb that breathes + shifts color with live state — working / thinking / awake / resting) anchors the Dashboard and sidebar, with a persistent **activity ticker** on every page so progress is ambient. Pages: dashboard command-center (live activity timeline + mind/goals/budget, auto-refreshing), real-time chat with multi-conversation history, **Companies** (ABE list + per-company ledger / voice / strategy + blockers / pending drafts), **Goals** (queue + checkpoints), **Roles**, **Affect** (PAD bars + the behavioral cue the state imposes + recent events), **Ego** (Higgins actual/ideal/ought, coherence, per-capability confidence, humbling events), tools & skills browser, knowledge viewer, autonomous-mind monitor (live events + start/stop), schedule manager (with enable/disable/delete), channels, settings, and history. Long lists paginate; schedules support delete + toggle. Pick from 4 **themes** (Light / Dark / Nocturne / Mocha) in the sidebar or Settings → Appearance. Launch with `./start.sh --web`
...
- **Autonomous goal loop** — decompose complex goals into checkpoints, track progress across sessions, self-evaluate and revise plans. Background execution with auto-resume on restart. **Dream phase v2** when no goals exist: seven-lens daily rotation (compounding / capability / research / relational / creation / identity / infrastructure), persistent dream journal so cycles can't re-propose ideas already considered, skill-aware context, environmental snapshot (affect / ego / planning state), pre-persist cosine-dedup against existing goals. **Goal-completion → instant dream wakeup** so finishing a goal kicks the next dream cycle in seconds, not hours. **Mechanical FORCE-DREAM gate** prevents the LLM from wandering off when zero workable goals exist. **Planning-stuck retry** unsticks goals that decompose never produced. Full goal lifecycle: create, pause, resume, cancel, delete, delete_all
...
| Knowledge | knowledge_search, knowledge_write, knowledge_index, skill_read, skill_list, skill_promote | 6 |
...
pytest tests/ -v                   # Run tests (2080 passing)
ruff check .                       # Lint
```
